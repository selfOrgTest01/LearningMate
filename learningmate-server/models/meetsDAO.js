...
const meetsDAO = {
  ...
  insert: async (item, callback) => { // data 들어온거 확인 완료!
    try {
      const resp = await db.query(sql.insert, [item.meet_id, item.title, item.content, item.start_date, item.end_date, item.max_num, item.onoff, item.image, item.category, item.approve, item.user_id]);
