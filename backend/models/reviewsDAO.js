...
const reviewsDAO = {
  reviewList: async (meet_id, callback) => {
    ...
    try {
      ...
      if (resp.length === 0) {
        ...
        callback({status: 500, message: '모임 상세 조회 실패', error: '모임이 없습니다'});
      } else {
        callback({status: 200, message: '모임 상세 조회 성공', data: resp[0]});
      }
    } catch (error) {
      console.error(error);
      callback({status: 500, message: '모임 상세 조회 실패', error: error.message});
    }
  },
  ...
  participantActive: async (user_id) => {
    try {
      const [result] = await db.query('SELECT status FROM meet_participants WHERE user_id = ?', [
        user_id,
      ]);
      console.log(result);
  ...
  checkReviewAuthor: async (review_id, user_id) => {
    try {
      const [result] = await db.query('SELECT user_id FROM meet_reviews WHERE review_id = ?', [
        review_id,
      ]);
  ...
  delete: async (review_id, user_id, callback) => {
    // 데이터 확인
    try {
