...
---- 2023.12.22 수정 - 모임 리스트 DB, CRUD 완료
---- 2023.12.26 수정 - 모임, 참여자 리스트 DB, CRUD 완료


------------------------ meet table ------------------------
DROP TABLE meets;
-- 모임을 생성할때 생성하는 user가 manager가 1이 돼야함
...
-- 현재 제약을 삭제
ALTER TABLE meets
DROP FOREIGN KEY meet_user_id_fk;

-- 새로운 제약을 추가하고 ON DELETE CASCADE 옵션을 설정
ALTER TABLE meets
ADD CONSTRAINT meet_user_id_fk
FOREIGN KEY (user_id)
...
-- READ 읽기
SELECT * FROM meets;
SELECT * FROM meets WHERE meet_id = 14;

-- 전체 조회 (GROUP BY 해야하나?)
SELECT m.meet_id, u.user_id, u.nickname, title, DATE_FORMAT(m.createdAt, '%Y-%m-%d') as createdAt
...
SELECT m.meet_id, u.nickname, title, content, start_date, end_date, max_num, onoff, image, category, approve, DATE_FORMAT(m.createdAt, '%Y-%m-%d') as createdAt
FROM users u INNER JOIN meets m ON u.user_id = m.user_id
WHERE m.meet_id = 14;
-- 닉네임 가져와지는 거 확인
...
CREATE TABLE meet_participants (
  ...
  status            BOOLEAN     NOT NULL     DEFAULT 0, -- 참여 여부
  CONSTRAINT meet_participants_participant_id_pk PRIMARY KEY(participant_id),
  CONSTRAINT meet_participants_meet_id_fk FOREIGN KEY(meet_id) REFERENCES meets(meet_id), -- meet 테이블의 meet_id를 참조
  CONSTRAINT meet_participants_user_id_fk FOREIGN KEY(user_id) REFERENCES users(user_id)
  ON DELETE CASCADE
);

-- 방금 추가한 모임의 meet_id 가져오기
SET @last_meet_id = LAST_INSERT_ID();
...
SELECT p.participant_id, m.meet_id, u.user_id, u.nickname, p.status, 
  CASE
...
UPDATE meet_participants SET status = 1 WHERE participant_id = 3;
...
-- manager가 1인 사람만 삭제할 수 있게
DELETE FROM meet_participants WHERE participant_id = ? AND meet_id = ? AND manager = 1
...
-- status 제한 두기 .. question.session.? api 생성... 모임
-- 파라메터 동적으로 붙게 meet_id....?
-- meet_id 1234, -> user params?
...
CREATE TABLE meet_reviews (
  ...
  CONSTRAINT meet_reviews_meet_id_fk FOREIGN KEY(meet_id) REFERENCES meets(meet_id), -- meet 테이블의 meet_id를 참조
  CONSTRAINT meet_reviews_user_id_fk FOREIGN KEY(user_id) REFERENCES users(user_id)
);

SELECT * FROM meet_reviews;

---- TEST
-- CREATE 리뷰 추가
-- inner join 사용? status
INSERT INTO meet_reviews (review_id, meet_id, user_id, content)
VALUES (1, 1, 17, '리뷰 내용 1'),
       (2, 1, 19, '리뷰 내용 2'),
       (3, 1, 20, '리뷰 내용 3');

-- READ 읽기
SELECT * FROM meet_reviews;
SELECT * FROM meet_reviews WHERE review_id = 1;

-- 게시물 당 리뷰 전체 조회 (1번 게시물에 있는 모든 리뷰)
SELECT r.review_id, m.meet_id, u.nickname, r.content
...
SELECT r.review_id, m.meet_id, u.nickname, r.content
FROM meet_reviews r INNER JOIN users u ON r.user_id = u.user_id INNER JOIN meets m ON r.meet_id = m.meet_id
WHERE m.meet_id = 1 AND r.review_id = 1;

-- DELETE 특정 리뷰 삭제 (리뷰를 작성한 사용자만 삭제 가능)
DELETE FROM meet_reviews
WHERE review_id = 1
  AND user_id = 17;
...
-- 권한
-- parsist? 팔시스트? redux 에 저장하면 사용할때 상관없을것같은 느낌? redux 관리자 화면 가져오고 